...
// (API HTTPS en vez de SSH). Se busca la ONU por la "IP asignada" del
// cliente — ver src/lib/oltUbiquiti.js.
async function consultarOltUbiquiti(cliente) {
  if (!cliente.ip_asignada) {
    return {
      error: 'Este cliente no tiene "IP asignada" configurada en su ficha — es el dato que se usa para encontrar su ONU en esta OLT.',
    };
  }
  try {
...
export async function POST(request) {
  ...
  const { data: cliente, error: errCliente } = await auth.supabaseAdmin
    .from('clientes')
    .select('nombre, pppoe_usuario, ciudad, olt_puerto_pon, olt_onu_id, olt_marca, ip_asignada')
    .eq('id', clienteId)
    .single();
